...
struct PARENTRY ip_pars[]={	{parse_mseparator,"_:",ip_host},
...
void bg_init(int idx, void *d){printf("init test driver\n");}
int bg_open(int ix,void **priv)
{
  printf("test channel opened:\n");
  printf("\t parsed values: %d,%c,%d,%d,%d %s\t%s\n",bg_par[0].i,bg_par[1].i>31 ?bg_par[1].i : '.',bg_par[2].i,bg_par[3].i,bg_par[4].i,bg_par[5].s,bg_par[6].s);
...
void bg_io(int ix, void *priv){*reg=QERR_BP;}
#endif
