...
screen_specs qlscreen={128*1024, 128*1024+32*1024, 0x8000, 128, 256, 512};
...
next_change:
  ...
  ii=i;
  dst_scrdiff=(char*)oldscr+qlscreen.qm_len/*+2*sizeof(long)*/;
  while( (char *)oscr<(char*)oldscr+qlscreen.qm_len/*+2*sizeof(long)*/ &&
	 *ascr++!=*oscr++ ) 
  {
	  dst_scrdiff=((char*)oldscr+qlscreen.qm_len)-(char *)oscr;
    *(oscr-1)=*(ascr-1);    /* actualise cmp/shadow buffer */
  }
  printf("scrconv end of diff area: page %d\trealpage %l\toffset %l\n",i,(((int)((char*)oscr-oldscr))/pagesize),(char*)oscr-oldscr-pagesize*(((int)((char*)oscr-oldscr))/pagesize));
